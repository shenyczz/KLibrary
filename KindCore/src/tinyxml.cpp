...
{
	TiXmlBase::condenseWhiteSpace = condense;
}

// static
bool TiXmlBase::IsWhiteSpaceCondensed()
{
	return TiXmlBase::condenseWhiteSpace;
}
...
void TiXmlDeclaration::Print( FILE* cfile, int /*depth*/, TIXML_STRING* str ) const
{
	...
	if ( !m_version.empty() ) {
		if ( cfile ) fprintf (cfile, "version=\"%s\" ", m_version.c_str ());
		if ( str ) { (*str) += "version=\""; (*str) += m_version; (*str) += "\" "; }
	}
	if ( !m_encoding.empty() ) {
		if ( cfile ) fprintf (cfile, "encoding=\"%s\" ", m_encoding.c_str ());
		if ( str ) { (*str) += "encoding=\""; (*str) += m_encoding; (*str) += "\" "; }
	}
	if ( !m_standalone.empty() ) {
		if ( cfile ) fprintf (cfile, "standalone=\"%s\" ", m_standalone.c_str ());
		if ( str ) { (*str) += "standalone=\""; (*str) += m_standalone; (*str) += "\" "; }
	}
