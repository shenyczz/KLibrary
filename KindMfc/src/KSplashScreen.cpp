...
IMPLEMENT_DYNAMIC(KSplashScreen, CWnd)
...
KSplashScreen::KSplashScreen()
{
	...
	if (hUser32!=NULL)
		m_fnAnimateWindow = (FN_ANIMATE_WINDOW)GetProcAddress(hUser32, (LPCSTR)(LPCTSTR)_T("AnimateWindow"));
	else
...
//BOOL KSplashScreen::Create(CWnd *pWndParent,LPCTSTR szText,DWORD dwTimeout,DWORD dwStyle)
BOOL KSplashScreen::Create(HWND hWndParent,LPCTSTR szText,DWORD dwTimeout,DWORD dwStyle)
{
	...
	wcx.lpszClassName = _T("SfxSplashScreenClass");
	wcx.hIconSm = NULL;
	...
	if (!CreateEx(WS_EX_TOOLWINDOW | WS_EX_TOPMOST,_T("SfxSplashScreenClass"),NULL,WS_POPUP,0,0,0,0,m_hWndParent,NULL))
		return FALSE;

	return TRUE;
}

BOOL KSplashScreen::SetBitmap(UINT nBitmapID,short red,short green,short blue)
{
	BITMAP bm;
	...
	else if (m_dwStyle & CSS_CENTERAPP)
	{
		...
		::GetWindowRect(m_hWndParent,&rcParentWindow);
...
BOOL KSplashScreen::SetBitmap(LPCTSTR szFileName,short red,short green,short blue)
{
	BITMAP bm;
	HBITMAP hBmp;
...
void KSplashScreen::SetTextFont(LPCTSTR szFont,int nSize,int nStyle)
{
	LOGFONT lf;
...
void KSplashScreen::SetTextDefaultFont()
{
	LOGFONT lf;
...
void KSplashScreen::SetText(LPCTSTR szText)
{
	m_strText=szText;
	RedrawWindow();
}

void KSplashScreen::SetTextColor(COLORREF crTextColor)
{
	m_crTextColor=crTextColor;
	RedrawWindow();
}

void KSplashScreen::SetTextRect(CRect& rcText)
{
	m_rcText=rcText;
	RedrawWindow();
}

void KSplashScreen::SetTextFormat(UINT uTextFormat)
{
	m_uTextFormat=uTextFormat;
}

void KSplashScreen::Show()
{
...
HRGN KSplashScreen::CreateRgnFromBitmap(HBITMAP hBmp, COLORREF color)
{
...
void KSplashScreen::DrawWindow(CDC *pDC)
{
	CDC memDC;
	CBitmap *pOldBitmap;
...
BEGIN_MESSAGE_MAP(KSplashScreen, CWnd)
	ON_WM_ERASEBKGND()
	ON_WM_PAINT()
...
// KSplashScreen message handlers

BOOL KSplashScreen::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}

void KSplashScreen::OnPaint()
{
	CPaintDC dc(this); // device context for painting
	DrawWindow(&dc);
}

LRESULT KSplashScreen::OnPrintClient(WPARAM wParam, LPARAM lParam)
{
...
BOOL KSplashScreen::PreTranslateMessage(MSG* pMsg)
{
...
void KSplashScreen::OnTimer(UINT nIDEvent)
{
	KillTimer(0);
	Hide();
}

void KSplashScreen::PostNcDestroy()
{
	CWnd::PostNcDestroy();
	delete this;
}
